...
from energy_cluster_line import line_curve


def curve_line(label_x_t, label_y_t, label_x_q, label_y_q, item_id):
    if len(label_x_t) > 0:

        z1 = line_curve(label_x_t,label_y_t)
        p1 = np.poly1d(z1)
        print(p1)  # 在屏幕上打印拟合多项式
        yvals = p1(label_x_t)  # 也可以使用yvals=np.polyval(z1,x)

        plot1 = plt.plot(label_x_t, label_y_t, 'o', label='original train values')
...
if __name__ == "__main__":
    ...
    train_file, query_file, points_num = data_path + "sift_train10k_test.bin", data_path + "sift_query_test.bin", 1000000
